...
|---|---|---|---|
| Alex Lee | Finance Analyst | Finance | Your primary test user — beginner labs |
| Morgan Chen | Software Engineer | Engineering | Group and app access labs |
| Jordan Kim | IT Administrator | IT | PIM and privileged access labs |
| Sam Taylor | Contractor | External | Governance and expiry labs |
| River Patel | HR Manager | HR | Lifecycle and provisioning labs |
...
| 2 | Users and Groups | 6 | Create, manage, and organize identities at scale |
| 3 | Authentication | 6 | Configure MFA, passwordless, SSPR, and authentication policies |
| 4 | Conditional Access | 7 | Build and manage a complete CA policy stack |
| 5 | Applications and SSO | 8 | Connect SaaS apps, configure SSO, manage permissions |
...
| 8 | Identity Governance | 6 | Run access reviews, entitlement management, lifecycle automation |
| 9 | Devices and Compliance | 7 | Enroll devices, enforce compliance, integrate with Intune |
| 10 | Hybrid Identity | 6 | Connect on-premises AD to Entra ID |
| 11 | Workload Identities | 7 | Secure apps and automation without passwords |
| 12 | Monitoring and Operations | 6 | Read logs, troubleshoot failures, build runbooks |
...
*Learn to create, organize, and manage the identities that everything else in Entra ID is built on.*

> **SC-300 mapping:** Domain 1 — Implement identities in Entra ID
...
### 3.1 — How Sign-In Works — The Flow from Start to Finish
**You will learn:** What happens at every step of a sign-in — from the user typing their email to receiving an access token. Where authentication ends and authorization begins. Why this flow matters for troubleshooting.

**Lab:** Sign in as Alex Lee and trace every step in the sign-in log. Match each log field to a step in the flow. Simulate a partial failure (wrong password) and find it in the logs. Simulate a complete failure (disabled account) and compare the error.

**You'll be able to:** Read any sign-in log entry and describe what happened at each step.
...
### 3.6 — Lab: Secure Your Tenant's Authentication — End to End
...
### 4.1 — What Conditional Access Is — The IF-THEN Engine
...
### 4.2 — Your First Policy — Require MFA for Administrators
...
### 4.3 — Named Locations — Trusted and Untrusted Networks
...
### 4.4 — Device Conditions — Managed vs Unmanaged
...
### 4.5 — App Conditions — Not All Apps Are Equal
...
### 4.6 — Report-Only Mode and Testing Safely
...
### 4.7 — Lab: Build a Production-Ready CA Policy Stack
...
> **SC-300 mapping:** Domain 3 — Implement access management for applications
...
**Lab:** No configuration lab — this module is conceptual. Work through each attack scenario: given this attack method, which Entra ID control stops it, and why does it stop it? Build your own attack-defense reference table.

**You'll be able to:** Explain to a security team what each attack does and which specific Entra ID control mitigates it.
...
> **SC-300 mapping:** Domain 4 — Plan and implement identity governance
...
**Lab:** Enable the CA policy requiring compliant device for SharePoint (built in Unit 4 — was in Report-Only). Now that you have a compliant device, test it: sign in from your compliant device (allowed), sign in from an incognito browser simulating an unmanaged device (blocked). Read the CA result in the sign-in log for each.
...
> **Estimated time:** 7–8 hours (includes Azure VM lab setup)

---

### 10.1 — Why Hybrid Identity Exists
...
### 10.3 — Authentication Methods — PHS, PTA, and Federation
...
### 10.4 — Source of Authority — What to Edit Where
...
### 10.5 — Troubleshooting Sync Errors
...
> **SC-300 mapping:** Domain 3 — workload identities
> **Estimated time:** 6–7 hours
...
**You will learn:** What Workload Identity Federation is, how GitHub Actions, Google Cloud, AWS, and Kubernetes can authenticate to Entra ID using their own OIDC tokens without any stored secrets, how federated credentials work.

**Lab:** Create an app registration for a GitHub Actions integration. Add a federated credential pointing to your GitHub repository. Configure a GitHub Actions workflow using `azure/login@v2` with OIDC (no secrets). Run the workflow — watch it authenticate to Entra ID and call Microsoft Graph without any secret in GitHub.

**You'll be able to:** Configure Workload Identity Federation for GitHub Actions.
...
|---|---|---|
| Implement identities in Entra ID | 25-30% | Units 2, 10 |
| Implement authentication and access management | 25-30% | Units 3, 4, 6, 9 |
| Implement access management for applications | 15-20% | Units 5, 11 |
| Plan and implement identity governance | 20-25% | Units 7, 8 |
...
- SC-300 exam study guide: `learn.microsoft.com/certifications/exams/sc-300`
...
> **Entra ID — Zero to Admin** · 13 Units · 75 Modules · Concept + Lab + Scenario
>
